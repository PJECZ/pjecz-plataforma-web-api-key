...
def get_siga_salas(
    ...
    consulta = database.query(SIGASala)
...
def get_siga_sala(database: Session, siga_sala_id: int) -> SIGASala:
    """Consultar una sala por su id"""
    siga_sala = database.query(SIGASala).get(siga_sala_id)
...
def get_siga_sala_with_clave(database: Session, siga_sala_clave: str) -> SIGASala:
    """Consultar una sala por su clave"""
    try:
        clave = safe_clave(siga_sala_clave)
    except ValueError as error:
        raise MyNotValidParamError(str(error)) from error
    siga_sala = database.query(SIGASala).filter_by(clave=clave).first()
    if siga_sala is None:
        raise MyNotExistsError("No existe ese sala")
    if siga_sala.estatus != "A":
        raise MyIsDeletedError("No es activo ese sala, está eliminado")
