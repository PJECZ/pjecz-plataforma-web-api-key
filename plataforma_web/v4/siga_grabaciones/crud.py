...
def get_siga_grabaciones(
    ...
    consulta = database.query(SIGAGrabacion)
    ...
    if siga_sala_id is not None:
        siga_sala = get_siga_sala(database, siga_sala_id)
...
def get_siga_grabacion(database: Session, siga_grabacion_id: int) -> SIGAGrabacion:
    """Consultar una grabacion por su id"""
    siga_grabacion = database.query(SIGAGrabacion).get(siga_grabacion_id)
...
def get_siga_grabacion_with_archivo_nombre(database: Session, archivo_nombre: str) -> SIGAGrabacion:
    ...
    siga_grabacion = database.query(SIGAGrabacion).filter_by(archivo_nombre=archivo_nombre).first()
    if siga_grabacion is None:
        raise MyNotExistsError("No existe ese grabacion")
    if siga_grabacion.estatus != "A":
        raise MyIsDeletedError("No es activo ese grabacion, está eliminado")
...
def create_siga_grabacion(database: Session, siga_grabacion: SIGAGrabacion) -> SIGAGrabacion:
    """Crear una grabacion"""

    # Validar autoridad, si viene la clave definir el id de la autoridad
    if siga_grabacion.autoridad_id is not None:
        get_autoridad(database, siga_grabacion.autoridad_id)
    elif siga_grabacion.autoridad_clave is not None:
        autoridad = get_autoridad_with_clave(database, siga_grabacion.autoridad_clave)
    ...
    if siga_grabacion.materia_id is not None:
        get_materia(database, siga_grabacion.materia_id)
    elif siga_grabacion.materia_clave is not None:
        materia = get_materia_with_clave(database, siga_grabacion.materia_clave)
    ...
    if siga_grabacion.siga_sala_id is not None:
        get_siga_sala(database, siga_grabacion.siga_sala_id)
    elif siga_grabacion.siga_sala_clave is not None:
        siga_sala = get_siga_sala_with_clave(database, siga_grabacion.siga_sala_clave)
    ...
    # Guardar
    database.add(siga_grabacion)
    database.commit()
    database.refresh(siga_grabacion)
