...
def get_inv_modelos(
    ...
    consulta = database.query(InvModelo)
    if inv_marca_id:
        inv_marca = get_inv_marca(database, inv_marca_id=inv_marca_id)
...
def get_inv_modelo(database: Session, inv_modelo_id: int) -> InvModelo:
    """Consultar un modelo por su id"""
    inv_modelo = database.query(InvModelo).get(inv_modelo_id)
