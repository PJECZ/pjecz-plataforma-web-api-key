...
materias_tipos_juicios = APIRouter(prefix="/v3/materias_tipos_juicios", tags=["categoria"])
...
async def listado_materias_tipos_juicios(
    ...
    try:
        resultados = get_materias_tipos_juicios(db=db)
    except MyAnyError as error:
        return custom_page_success_false(error)
    return paginate(resultados)


@materias_tipos_juicios.get("/{{materia_tipo_juicio:singular}_id}", response_model=OneMateriaTipoJuicioOut)
